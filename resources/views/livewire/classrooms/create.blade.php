            ...
            <div class="modal-body">
                <form>
                    ...
                    <div class="form-group mt-3">
                        ...
                        <input wire:model="classroom_unique_id" readonly type="text" class="form-control"
                               id="classroom_unique_id" placeholder="{{__('ID Lớp Học')}}">@error('classroom_unique_id')
                        ...
                        <div>
                            <div>
                                @foreach ($allStudents as $student)
                                    <div>
                                        <input type="checkbox" wire:model="inputStudent.{{$student->id}}"
                                               value="{{$student->id}}"/>
                                        <label style="font-size: 13px; " for="vehicle1">{{ $student->name }}
                                            | {{ $student->email }}</label>
                                    </div>
                                @endforeach
                            </div>
